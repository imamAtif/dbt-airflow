...
### Troubleshooting: `manifest.json` not found

`dbt-airflow` requires an existing `manifest.json` file generated by dbt.

Before running the Airflow DAG, make sure you have executed a dbt command that creates the manifest file, for example:
...
By default, dbt writes the manifest file to:

```text
target/manifest.json
```

Make sure the `manifest_path` value in `DbtProjectConfig` points to the generated `manifest.json` file.
